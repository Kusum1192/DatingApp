...
public class BlockUserListActivity extends AppCompatActivity {
    ...
    SpinKitView loadBar;
    RecyclerView recylerview_block_user;
    BlockListAdapter blockListAdapter;
    ConstraintLayout no_block_list;
    Toolbar toolbar;
    ImageView backBtn;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recylerview_block_user = findViewById(R.id.recylerview_block_user);
        loadBar = findViewById(R.id.spin_kit);
        no_block_list = findViewById(R.id.no_block_list);
        toolbar = findViewById(R.id.blockToolbar);
        ...
        getSupportActionBar().setTitle(Html.fromHtml("<font color='#000'>Block User list</font>"));
        ...
        upArrow.setColorFilter(getResources().getColor(R.color.select_color), PorterDuff.Mode.SRC_ATOP);
...
*/
    ...
    private void BlockUserList() {
        ...
        Call<BlockUserListModels> call = service.blockUserList(Constants.getSharedPreferenceInt(this, "userId", 0),
                Constants.getSharedPreferenceString(this, "securityToken", ""),
                BuildConfig.VERSION_NAME,BuildConfig.VERSION_CODE);

        loadBar.setVisibility(View.VISIBLE);

        call.enqueue(new Callback<BlockUserListModels>() {
            @Override
            public void onResponse(Call<BlockUserListModels> call, final Response<BlockUserListModels> response) {
                dismissProgressDialog();
                if (response != null) {
                    if (response.isSuccessful()) {
                        if (response.body().getStatus() == 200) {
                            List<BlockUserListModels.BlockList> blockUserListModelsArrayList = response.body().getBlockList();

                            if (blockUserListModelsArrayList.size() > 0) {
                                blockListAdapter = new BlockListAdapter(blockUserListModelsArrayList, BlockUserListActivity.this);
                                recylerview_block_user.setLayoutManager(new LinearLayoutManager(BlockUserListActivity.this));
                                recylerview_block_user.setAdapter(blockListAdapter);
                            } else {
                                no_block_list.setVisibility(View.VISIBLE);
                            }

                        }
                    }
                } else {
                    Toast.makeText(BlockUserListActivity.this, "" + response.errorBody(), Toast.LENGTH_SHORT).show();
                }

            }

            @Override
            public void onFailure(Call<BlockUserListModels> call, Throwable t) {
    ...
    public class BlockListAdapter extends RecyclerView.Adapter<BlockListAdapter.ViewHolder> {
        ...
        public void removeAt(int position) {
            if (listCard.size() > 0) {
                unblockUser(context, listCard.get(position).getMatchId(), "unblock",position);
            } else {
                no_block_list.setVisibility(View.VISIBLE);
            }
        }
